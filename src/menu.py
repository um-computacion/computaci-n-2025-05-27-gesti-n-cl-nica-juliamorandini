...
class Menu:
    ...
    def menu():
        ...
        while True:
            ...
            print("0) Salir")
            opcion = input("Seleccione una opción: ") #queria ponelo que solo fuera int pero me da erroor

            try:
                if opcion == "1":  # agrega paciente
                    ...
                    dias = input("Días de atención por ejemplo: lunes, miércoles: ").lower().replace(" ", "").split(",")
                ...
                elif opcion == "6":  # ve historia clínica
                    dni = input("DNI del paciente: ")
                    historia = clinica.obtener_historia_clinica(dni)
                    if historia:  # si existe, se muestra, sino el print de abajo
                        print(historia)
                    else:
                    ...
                    if turnos:
                        for t in turnos:  # si está lo muestra, sino se va al print de abajo
                            print(t)
                    else:
...
def pedir_fecha_hora():
    while True:
        fecha_str = input("Ingrese fecha y hora (formato: dd/mm/aaaa Hora:Minuto): ")
        try:
